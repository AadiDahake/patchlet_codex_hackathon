...
`apps/web/lib/opportunity/model.ts` (`gpt-5.6-luna` for goal inference, `gpt-5.6-sol` for the
reward model and the naming call) over the sessions `apps/web/lib/posthog` mined, writes each event as a `capability`
